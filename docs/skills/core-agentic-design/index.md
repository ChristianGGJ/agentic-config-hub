---
title: "Core Agentic Design Skills — Agent Skills & Codex Plugins"
description: "7 core agentic design skills — core agentic design patterns and loop safety audits for autonomous systems. Works with Claude Code, Codex CLI, Gemini CLI, and OpenClaw."
---

<div class="domain-header" markdown>

# :material-robot: Core Agentic Design

<p class="domain-count">7 skills in this domain</p>

</div>

<div class="install-banner" markdown>
<span class="install-label">Install all:</span> <code>claude /plugin install core-agentic-design</code>
</div>

<div class="grid cards" markdown>

-   **[Agent Designer - Multi-Agent System Architecture](agent-designer.md)**

    ---

    Tier: POWERFUL

-   **[Agent Workflow Designer](agent-workflow-designer.md)**

    ---

    Tier: POWERFUL

-   **[AgentHub — Multi-Agent Collaboration](agenthub.md)**

    ---

    Spawn N parallel AI agents that compete on the same task. Each agent works in an isolated git worktree. The coordinat...

-   **[Agentic System Architect - Four-Pillar Ecosystem Design](agentic-system-architect.md)**

    ---

    Tier: POWERFUL

-   **[CrewAI Role Engineering](crewai-role-engineering.md)**

    ---

    Design CrewAI agent teams whose behavior is shaped by engineered personas, whose

-   **[Loop Engineering Mechanisms](loop-engineering-mechanisms.md)**

    ---

    This skill is the runnable implementation companion to the hub's loop-engineering

-   **[Spec-Driven Workflow — POWERFUL](spec-driven-workflow.md)**

    ---

    Spec-driven workflow enforces a single, non-negotiable rule: write the specification BEFORE you write any code. Not a...

</div>
