---
name: "plan-baseline-tracking"
description: "Use when tracking an approved project plan against reality -- diffing an immutable baseline plan.json against an append-only status.jsonl ledger to compute per-task start/finish variance in working days, percent-complete vs expected, DCMA-style schedule health checks, and an overall schedule health verdict."
---
