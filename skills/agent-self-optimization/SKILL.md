...
description: "Use when automatically optimizing an agent's prompts or policy from an error, audit, or eval signal under a mandatory human gate: selecting a DSPy-style optimizer (few-shot bootstrapping vs instruction search vs full compile), turning captured failures into a governed prompt rewrite, or wiring in-context corrective feedback / RLAIF vs RLHF signals into a versioned config. NOT for defining the metric or golden set (use agentic-evals-benchmarking). NOT for the prompt registry, versioning, promotion, or rollback (use prompt-governance). NOT for the autonomous experiment harness (use autoresearch-agent). NOT for loop-safety counters (use loop-engineering-mechanisms). NOT for hand-writing a single prompt (use senior-prompt-engineer)."
---
