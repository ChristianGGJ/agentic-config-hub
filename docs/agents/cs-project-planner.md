---
title: "Project Planner — AI Coding Agent"
description: "Governed project-planning orchestrator for the full planning lifecycle - requirements elicitation, WBS decomposition, dependency and critical-path. Agent-native orchestrator for Claude Code."
---
...
<span class="meta-badge">:material-github: <a href="https://github.com/ChristianGGJ/agentic-config-hub/tree/main/agents\cs-project-planner.md">Source</a></span>
</div>
...
1. **sequential-elicitation** - [`skills\sequential-elicitation\scripts\elicitation_ledger.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\sequential-elicitation\scripts\elicitation_ledger.py)
   drives a bounded question agenda to closure and records answers in an auditable ledger.
2. **stakeholder-inference** - [`skills\stakeholder-inference\scripts\stakeholder_register_validator.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\stakeholder-inference\scripts\stakeholder_register_validator.py)
   builds and validates a stakeholder register (roles, influence, engagement).
3. **wbs-decomposition** - [`skills\wbs-decomposition\scripts\wbs_validator.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\wbs-decomposition\scripts\wbs_validator.py) validates the WBS
   against the 100-percent rule and emits hub-contract leaf tasks (`--emit-tasks plan.json`).
4. **critical-path-scheduler** - [`skills\critical-path-scheduler\scripts\cpm_scheduler.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\critical-path-scheduler\scripts\cpm_scheduler.py) validates
   the dependency graph and runs CPM forward/backward passes for dates, float, and the critical path.
5. **plan-critique** - [`skills\plan-critique\scripts\plan_audit.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\plan-critique\scripts\plan_audit.py) runs hostile pre-execution
   review with severity-classified findings and a min-score gate.
6. **plan-premortem** - [`skills\plan-premortem\scripts\premortem_register_validator.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\plan-premortem\scripts\premortem_register_validator.py) and
   [`skills\plan-premortem\scripts\scenario_matrix_expander.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\plan-premortem\scripts\scenario_matrix_expander.py) validate the premortem register and
   expand failure-scenario matrices.
7. **blind-spot-audit** - [`skills\blind-spot-audit\scripts\coverage_gap_scanner.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\blind-spot-audit\scripts\coverage_gap_scanner.py) scans the brief
   and plan for whole missing dimensions against coverage profiles.
8. **plan-baseline-tracking** - [`skills\plan-baseline-tracking\scripts\baseline_variance.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\plan-baseline-tracking\scripts\baseline_variance.py) detects
   schedule variance of a live plan against its baseline (MONITOR mode).
9. **slip-driven-replanning** - [`skills\slip-driven-replanning\scripts\slip_injector.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\slip-driven-replanning\scripts\slip_injector.py) and
   [`skills\slip-driven-replanning\scripts\replan_impact.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\slip-driven-replanning\scripts\replan_impact.py) inject a slip and compute milestone impact.
10. **plan-ticket-export** - [`skills\plan-ticket-export\scripts\ticket_payload_generator.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\plan-ticket-export\scripts\ticket_payload_generator.py) builds
...
- **RAG context ingestion is delegated, not built here.** This agent has **no RAG skill of its own**.
  ...
  it orchestrates **rag-architect** ([`skills\rag-architect\scripts\rag_pipeline_designer.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\rag-architect\scripts\rag_pipeline_designer.py)) to
  design the retrieval pipeline and **hybrid-rag-memory**
  ([`skills\hybrid-rag-memory\scripts\memory_evictor.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\hybrid-rag-memory\scripts\memory_evictor.py), references under
  [`skills\hybrid-rag-memory\references`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\hybrid-rag-memory\references)) for persistent long-term memory and BM25+vector hybrid
...
- **The critique-loop identity is delegated to the adversarial reviewer.** When a team requires an
  ...
  ([`skills\adversarial-reviewer\SKILL.md`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\adversarial-reviewer\SKILL.md)) and follows the episodic-critique method in the
  agentic-system-architect reference
  [`skills\agentic-system-architect\references\self_reflection_critique_loops.md`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\agentic-system-architect\references\self_reflection_critique_loops.md). Producers fix;
...
- **Premortem fan-out is delegated to agenthub.** When a premortem must be explored in parallel across
  ...
  ([`skills\agenthub\scripts\session_manager.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\agenthub\scripts\session_manager.py), [`skills\agenthub\scripts\board_manager.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\agenthub\scripts\board_manager.py),
  [`skills\agenthub\scripts\dag_analyzer.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\agenthub\scripts\dag_analyzer.py), [`skills\agenthub\scripts\result_ranker.py`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\agenthub\scripts\result_ranker.py)) to
...
- Planning-knowledge and templates live inside each owning skill's `references/` and `assets/`
  (for example [`skills\critical-path-scheduler`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\critical-path-scheduler), [`skills\wbs-decomposition\references\plan_json_contract.md`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\wbs-decomposition\references\plan_json_contract.md),
  [`skills\rag-architect\references\agentic_rag_patterns.md`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\rag-architect\references\agentic_rag_patterns.md),
  [`skills\hybrid-rag-memory\references\rag_memory_patterns.md`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\hybrid-rag-memory\references\rag_memory_patterns.md),
  [`skills\agenthub\references\dag-patterns.md`](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\agenthub\references\dag-patterns.md)). The agent points at them; it does not copy them in.
...
- [WBS Decomposition Skill](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\wbs-decomposition\SKILL.md)
- [Critical-Path Scheduler Skill](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\critical-path-scheduler\SKILL.md)
- [Plan Critique Skill](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\plan-critique\SKILL.md)
- [Self-Reflection & Critique Loops](https://github.com/ChristianGGJ/agentic-config-hub/tree/main/skills\agentic-system-architect\references\self_reflection_critique_loops.md)
