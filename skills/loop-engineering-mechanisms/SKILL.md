...
description: "Use when implementing loop safety in Python code: wiring all six canonical exit conditions (max_iterations, no_progress, oscillation, budget, success_predicate, escalation_trigger) into an agent loop, adding output validation gates before accepting an iteration, formatting machine-readable error observations, or producing structured stop-and-report exit handoffs."
---
